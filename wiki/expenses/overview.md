...
The Expense Service is a REST API for submitting and tracking employee expenses. It enforces per-category monthly spending caps and routes high-value expenses (> $2000) to the [expense-approval-service](https://github.com/kapsha/expense-approval-service) for manager sign-off before recording.

---

## Key Business Rules

1. **High-value expenses require notes.** Any expense with `amount > $2000` must include a non-blank `notes` field; the request is rejected at validation time (HTTP 422) if this is missing.
...
```json
{
  ...
  "notes":    "string | null (required when amount > $2000)"
}
```
...
| `notes`    | `string\|null` | Mandatory when amount > $2000  |
...
| Dependency | Purpose |
|---|---|
| [expense-approval-service](https://github.com/kapsha/expense-approval-service) | Receives high-value expenses (> $2000) for manager sign-off before they are recorded |
